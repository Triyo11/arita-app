  ...
  return (
    <header className="px-20 sticky top-14 z-10 bg-color-primary">
      <div className="md:px-8 md:py-2 p-4">
        <Link href={`/channel/${source}`}><ArrowUUpLeft size={48} className={`text-color-primary bg-color-secondary rounded-3xl p-2 fixed bottom-10 right-10 z-10 ${isMediumScreen ? "hidden" : "block"}`}/></Link>
        <h2 className="flex justify-center font-bold md:text-3xl text-2xl text-color-secondary">
          {headerTitle}
        </h2>
      </div>
    </header>
