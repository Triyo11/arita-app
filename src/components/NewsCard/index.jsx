  ...
  return (
    <div className="flex flex-wrap justify-evenly p-4 gap-8">
      {news.map((item, index) => {
        ...
        return (
          <div key={item.link} className="md:w-80 w-40 relative">
            <Image src={item.thumbnail} alt="gambar error" width={350} height={350} className="rounded-md"/>
            <div className="flex flex-col justify-start items-start py-4">
              <p>{pubDate}</p>
              <a href={item.link} className="text-lg font-semibold focus:text-color-accent2">{item.title}</a>
            </div>
